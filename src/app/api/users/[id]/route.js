...
export const DELETE = async (_, { params }) => {
  ...
  try {
    ...
    if (userIndex === -1) return NextResponse.json({ message: "Хэрэглэгч олдсонгүй" }, { status: 404 });
...
export const PUT = async (req, { params }) => {
  ...
  try {
    ...
    return NextResponse.json({ message: "Амжилттай шинэчиллээ", data: users[userIndex] });
  } catch (error) {
    return NextResponse.json({ message: "Алдаа гарлаа", error: error.message }, { status: 500 });
  }
};
